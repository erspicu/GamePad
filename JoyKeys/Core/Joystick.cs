        ...
        public bool Register(IntPtr hWnd, int joystickId)
        {
           ...
           // joystickId = 1;
            ...
            if (flag)
            {
                ...
                if (!this.IsRegister)
                {
                    Console.WriteLine("cccc 234");
                    Application.AddMessageFilter(this);
                }
                this.IsRegister = true;


                Console.WriteLine("register: "+ joystickId);
                result = API.joySetCapture(hWnd, 1 , 50, false);
                result = API.joySetCapture(hWnd, 2, 50, false);
                Console.ReadLine();
               // result = API.joySetCapture(hWnd, 2, 50, false);

                Console.WriteLine("param " + result.ToString() + " " + caps.wPeriodMin * 2);
